...
import { AddressAliasHelper } from "../../components/AddressAliasHelper";
import {
  MermaidWithHtml,
  Nodes,
  Node,
  ...
  NodeDescriptions,
  NodeDescription,
} from "../../components/MermaidWithHtml/MermaidWithHtml";
...
3. Ticket creation causes the [`ArbRetryableTx`](/for-devs/dev-tools-and-resources/precompiles.mdx#arbretryabletx) precompile to emit a `TicketCreated` event containing the `TicketID` on L2.
