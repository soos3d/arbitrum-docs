...
import sidebars from "../website/sidebars";
  ...
  switch (sidebarItem.type) {
    ...
    case "category":
      if (!sidebarItem.items) {
        console.warn(
          "Sidebar item of type category does not have items: " + sidebarItem
        );
        return;
      }
    ...
    case "autogenerated":
      if (!sidebarItem.dirName) {
        console.warn(
          "Sidebar item of type autogenerated does not have a dirName: " +
            sidebarItem
        );
        return;
      }
    ...
    default:
      console.warn(
        "Detected unhandled type on sidebar item: " + sidebarItem.type
      );
      return;
  }
};
...
const extractLinksFromMdFile = (filePath: string) => {
  ...
  links.forEach((link: string) => {
    ...
    if (link.startsWith("https://developer.offchainlabs.com")) {
      ...
      ["https://developer.arbitrum.io", "https://docs.arbitrum.io"].some(
  ...
  const importPathRegex =
    /import\s+[a-zA-Z_]\w+\s+from\s+'(?<path>.+\.mdx?)';/g;
...
resources.forEach((resourcePath: string) => {
  if (
    ...
    (!resourcePath.endsWith(".png") ||
      !resourcesLinkedInDocs.includes(resourcePath))
  ) {
