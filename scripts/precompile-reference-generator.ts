import globalVars from "../docs/resources/globalVars.js";
import {
  precompilesInformation,
  nodeInterfaceInformation,
} from "../docs/resources/precompilesInformation.js";
import fs from "fs";
...
const renderMethodsInTable = (
  ...
  interfaceCode.split("\n").forEach((line) => {
    ...
    if (trimmedLine.startsWith("function")) {
      const signature =
        trimmedLine.split(")")[0].replace("function", "").trim() + ")";
  ...
  implementationCode.split("\n").forEach((line) => {
    ...
    if (trimmedLine.startsWith("func")) {
      ...
      if (methodsInformation[methodName]) {
        methodsInformation[methodName].implementationLine =
          implementationLineNumber;
    ...
    <tbody>
      ${Object.values(methodsInformation)
        .map((methodInfo: PrecompileMethodInfo) => {
          ...
          return `<tr>
            <td>${methodInfo.deprecated ? `⚠️` : ""}<code>${
  ...
  implementationCode.split("\n").forEach((line) => {
    ...
    Object.keys(eventsInformation).map((eventName) => {
      if (line.toLowerCase().includes(`con.${eventName}(`)) {
        eventsInformation[eventName].implementationLine =
          implementationLineNumber;
      }
    });
  });
...
const generatePrecompileReferenceTables = async (
  ...
  fs.writeFileSync(
    `${partialTablesBasePath}/_${precompileName}.md`,
  ...
  await Promise.all(
    Object.keys(precompilesInformation).map(async (precompileName) => {
      const methodOverrides =
        precompilesInformation[precompileName].methodOverrides ?? undefined;
      const eventOverrides =
        precompilesInformation[precompileName].eventOverrides ?? undefined;
